...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/flare-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.lineage.device | grep -q "$FLARE_BUILD"); then
...
function flareremote()
{
    ...
    git remote rm flare 2> /dev/null
    ...
    local FLARE_USER=$(git config --get review.review.flareos.org.username)
    if [ -z "$FLARE_USER" ]
    then
        git remote add flare ssh://review.flareos.org:29418/$PFX$PROJECT
    else
        git remote add flare ssh://$FLARE_USER@review.flareos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'flare' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/FlareOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lineage.device | grep -q "$FLARE_BUILD");
    then
...
function flaregerrit() {
    ...
    local user=`git config --get review.review.flareos.org.username`
...
function flarerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "FlareOS Gerrit Rebase Usage: "
        echo "      flarerebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.flareos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$FLARE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/flare/build/tools/repopick.py $@
}
...
function build_kernel() {
    ...
    local flare_version="flare-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${flare_version}")
        if [ -n "${FLARE_MIRROR}" ]; then
            repo_init_args+=("--reference" "${FLARE_MIRROR}")
        fi
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/FlareOS/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
